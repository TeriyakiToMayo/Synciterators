...
# 
    ...
    def syncedWith(
      ...
      canSee = lambda y, x: True
        ...
        while self.hasNext():
            y, = self.peekahead()
            if isBefore(y, x) and not canSee(y, x):
                # Antimonotonicity condition 1:
                # bf(y, x) & !cs(y, x) => all x' after x: !cs(y, x')
                # So y can be discarded safely.
                self.next()
                continue
            elif not isBefore(y, x) and not canSee(y, x):
                ...
                # So got to insert these zs back to the front of 
                # the EIterator (viz. self.)
                self.prepend(zs)
                break
            elif len(self.lookahead(2)) < 2:
                # At this point, cs(y, x) is true, and there is no
                # more element on the EIterator (self).  So add this
                # element (y) to the output (zs). As the next x may
                # still be able to see some y saved earlier, got to 
                # insert zs back to the front of the EIterator (self.)
                self.prepend(zs)
                zs.append(y)
                break
            else:
                # At this point, cs(y, x) is true, and there is 
                # more element on the EIterator (self).  So add this
                # element (y) to the output (zs), and continue with
                # the next element on the EIterator.
                self.next()
                zs.append(y)
    ...
    def distinct(self):
        ...
        eit = ( e for e in self if [e] != self.peekahead(1) )
